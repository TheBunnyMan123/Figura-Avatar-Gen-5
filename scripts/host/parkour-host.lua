...
shift:setOnPress(function()
	...
	if not host:isFlying() and player:getVelocity().y > -2 and not player:getVehicle() and not player:isOnGround() and not player:isGliding() and not space:isPressed() then
...
function events.TICK()
	...
	if slamming then
		if player:isOnGround() and slamming >= 1 then
